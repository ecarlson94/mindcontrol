...
{	
  float4 emissiveMap = tex2D(EmissiveSampler, texCoord);
  
  // Convert from Gamma to linear space.
  emissiveMap.xyz = emissiveMap.xyz * emissiveMap.xyz;
  ...
  float3 diffuseLight = tex2D(LightBuffer0Sampler, texCoordScreen);
  float3 specularLight = tex2D(LightBuffer1Sampler, texCoordScreen);
