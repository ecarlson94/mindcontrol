...
// Copyright (C) Garstenauer Information Technology OG. All rights reserved.
//-----------------------------------------------------------------------------
//
/// \file Glass.fx
...
{
  half3 N = normalize(input.NormalWorld);
  ...
  // Reflection
  half3 R = reflect(-V, N);
  half4 reflectionColor = texCUBE(EnvironmentMapSampler, R) * ReflectionStrength;
  // Use a non-linear function to increase contrast of the reflections.
  reflectionColor = pow(reflectionColor, 2);
  
  // Refraction
  half4 refractionColor = 0;
  for(int i = 0; i < 3; i++)
  {
    half3 T = refract(-V, N, RefractionIndices[i]);
